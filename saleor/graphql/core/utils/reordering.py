...
class Reordering:
    ...
    @cached_property
    def nodes_pks_list(self):
        return list(self.ordered_node_map.keys())
    ...
    def resolve_node_from_global_id(self, global_id: str) -> int:
        ...
        if internal_type == self.model_name and _id.isnumeric():
            ...
            print(pk, self.ordered_node_map)
    ...
    def calculate_new_sort_order(self, pk, move) -> Optional[int]:
        ...
        node_pos = self.nodes_pks_list.index(pk)
        ...
        target_pos = min(len(self.nodes_pks_list) - 1, target_pos)

        # Retrieve the target node and its sort order
        target_pk = self.nodes_pks_list[target_pos]
    ...
    def process_move_operation(self, pk, move):
        ...
        new_sort_order = self.calculate_new_sort_order(pk, move)
        ...
        if move > 0:
            shift = -1
            range_ = old_sort_order, new_sort_order
        else:
            shift = +1
            range_ = new_sort_order, old_sort_order

        # Shift the sort orders within the moving range
        self.add_to_sort_value_if_in_range(shift, *range_)

        # FIXME: we need to keep the dictionary ordered
...
def perform_reordering(
    ...
    :raises ValueError: If a primary key is invalid. Will raise ValidationError if
    """
